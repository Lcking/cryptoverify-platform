...
1. 本地开发阶段
   - 前端：React 项目，使用 `npm ci && npm run build` 构建，环境变量控制 CMS 对接与 mock 回退。
   - 后端：Strapi CMS，Node 20，开发/测试用 SQLite，接口与内容类型按业务需求建模。
   - 本地联调：前端通过环境变量连接本地 Strapi，CORS 配置放开，Public 角色权限设置，API Token 测试。

2. 线上部署阶段
   - 前端：产物上传至服务器，Caddy 作为静态文件服务器，域名配置与 HTTPS 证书自动化。
   - 后端：Docker Compose 部署 Strapi，挂载代码目录与数据库卷，环境变量配置生产参数。
   - 反向代理：Caddy 配置 `X-Forwarded-Proto: https`，确保后端识别 HTTPS，前后端分域名隔离。

3. 生产环境联调
   - 前端：环境变量切换为线上 CMS 地址，重建并发布。
   - 后端：Public 角色权限放开，API Token 配置，接口联通性测试。
   - 数据校验：栏目、内容、权限、API 返回一致性检查。

## 二、Strapi 后台登录排查与修复经验

1. 问题现象
   - 线上 Strapi 管理后台登录始终 500，日志报错 `Cannot send secure cookie over unencrypted connection`。
   - 环境变量、配置文件、代理头均已设置，仍无法登录。

2. 排查过程
   - 逐步尝试：
     1. 配置 `proxy: true`、Caddy 转发 HTTPS 头 → 无效
     2. 环境变量禁用 secure → 无效
     3. 配置结构调整（`auth.sessions.cookie`）→ 无效
     4. 全局硬编码 secure: false → 无效
     5. Docker 环境变量 `NODE_TRUST_PROXY` → 无效
     6. **最终方案**：在 `src/index.ts` 注册 Koa 中间件，强制每个请求 `ctx.protocol = 'https'` → 登录成功
...
export default {
  register({ strapi }) {
    ...
    strapi.log.info('🔒 Protocol override middleware registered');
  },
  bootstrap() {},
};
```
...
4. 经验总结
   - Strapi 5.x 的 proxy 信任机制不完善，配置和环境变量无法彻底解决 HTTPS 检测问题。
   - 只有在 Koa 层强制覆盖协议，才能彻底绕过 secure cookie 检查。
   - 每次修改 `src/` 代码后必须清理 `dist/` 并重新编译。
   - 生产环境务必挂载数据库卷，避免数据丢失。
   - 代理/证书/反代变更后，务必验证后台登录。

5. 后续建议
   - 升级 Strapi 6.x 后需重新验证此问题。
   - 数据丢失/多余管理员账号需检查 Docker volume 挂载与数据库文件。
   - 将此经验纳入团队部署标准流程。

---

# 任务区隔线（以上为总结，以下为分任务记录）
